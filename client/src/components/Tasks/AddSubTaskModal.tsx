...
const AddSubTaskModal: React.FC<AddSubTaskModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({ taskData, endpoint: 'subtask' });
      setName('');
      setDescription('');
      setAreaId('');
      setProjectId('');
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="form-control">
          <label className="label">
            <span className="label-text text-white">Placeholder Task</span>
          </label>
          <select
            className="select bg-base-200/50 border-base-content/10"
            value={placeholderId}
            ...
            required
          >
            <option value="">Select a placeholder task</option>
            {placeholderTasks
              .filter(task => 
                (!task.area_id || task.area_id === areaId) &&
